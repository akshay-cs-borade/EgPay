...
task tshirts_collar_xl: :environment do
    store = Store.find_by(name: 'Renuka Mens Wear')
    size = Size.find_or_create_by!(name: 'XL')
    ...
    172.times do |i|
        ...
        barcode = Barcode.create!(
            barcode_number: product.generate_barcode(size.name, 'TShirt-Collar'),
            product: product,
            store: store
        )
